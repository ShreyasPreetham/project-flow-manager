 ...
 * Supports: title, description, assignee (required), stage, priority, due_date.
 */
export default function TaskForm({ initialData, onSubmit, onClose }) {
  ...
  const validate = () => {
    ...
    else if (form.assignee.trim().length > 120) errs.assignee = "Max 120 characters.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errs).length) {
      ...
      // Bring the user back to the first required field if modal was scrolled.
      if (errs.title) {
        ...
        >
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {STAGES.map((s) => (
                <label key={s}
                  className={`flex items-center justify-center px-3 py-2 rounded-lg border text-sm font-medium cursor-pointer transition
                    ${form.stage === s
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {PRIORITIES.map((p) => (
                <label key={p}
                  className={`flex items-center justify-center px-2 py-2 rounded-lg border text-xs font-semibold cursor-pointer transition
