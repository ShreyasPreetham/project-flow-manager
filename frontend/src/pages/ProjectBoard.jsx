...
export default function ProjectBoard() {
  ...
  const handleUpdate = async (id, data) => {
    const res = await api.put(`/projects/${projectId}/tasks/${id}/`, data);
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                {project?.project_type && (
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${TYPE_BADGE[project.project_type] || "bg-slate-100 text-slate-600"}`}>
                    {project.project_type}
                  </span>
                )}
              </div>
